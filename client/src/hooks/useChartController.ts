  ...
  const fetchData = useCallback(
    async (isRefreshing = false) => {
      ...
      if (!isRefreshing && queryKey === lastQueryRef.current && data !== null) {
        console.log(
          "[useChartController] Skipping fetch - query unchanged and data exists",
          { query },
        );
        return;
      }
      ...
      if (!isRefreshing && cacheRef.current.has(queryKey)) {
        ...
        console.log("[useChartController] Using cached data", {
          query,
          cachedData,
        });
        setData(cachedData);
      ...
      console.log("[useChartController] Fetching fresh data", {
        query,
        isRefreshing,
      });
