...
export function useProfileWalletTabData({
  walletAddresses,
  period,
}: UseProfileWalletTabDataInput): UseProfileWalletTabDataResult {
      ...
      try {
        ...
        const overviews = results
          .filter(
            (
              result,
            ): result is PromiseFulfilledResult<WalletOverviewMultiPeriodResponse> =>
        ...
        const portfolioRows = overviews.map((overview) => {
          const stats =
            overview.periods[periodKey] ??
            overview.periods[overview.selectedPeriod];
          const totalAssetValueUsd =
            overview.totalAssetValueUsd ??
            overview.holdings.totalAssetValueUsd ??
            0;
          const pnlUsd = stats?.pnl?.totalUsd ?? overview.pnlUsdTotal ?? 0;

          return {
            walletId: overview.address,
            walletLabel: fmt.text.address(overview.address),
            netWorthUsd: totalAssetValueUsd,
            pnlUsd,
            pnlPct:
              totalAssetValueUsd > 0 ? (pnlUsd / totalAssetValueUsd) * 100 : 0,
            tradeCount:
              stats?.transactionCount ?? overview.transactionCount24h ?? 0,
          };
        });

        const linkedWalletRows: LinkedWalletRow[] = overviews.map(
          (overview) => {
            const stats =
              overview.periods[periodKey] ??
              overview.periods[overview.selectedPeriod];
            ...
            const hasRecentActivity =
              (stats?.transactionCount ?? 0) > 0 || totalAssetValueUsd > 0;
        ...
        setData({
          ...
          portfolioRows,
          linkedWalletRows,
          selectedComparisonWalletIds: linkedWalletRows
            .slice(0, 2)
