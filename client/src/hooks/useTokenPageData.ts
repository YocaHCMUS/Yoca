        ...
        (async () => {
            ...
            const fetchBackendData = async () => {
                try {
                    ...
                    const [chartResp, marketResp, metaResp, holdersResp] = await Promise.all([
            ...
            const fetchClientData = async () => {
                try {
                    const { fetchTokenPools, fetchTokenHoldersInfo } = await import("../services/coingecko");
...
export const usePoolTrades = (network: string, poolAddress: string | undefined) => {
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            try {
                const response = await $getTrades({ param: { network, address: poolAddress } });
                if (response.ok) {
