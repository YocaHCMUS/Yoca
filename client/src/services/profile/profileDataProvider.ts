...
export interface ProfileDataProvider {
  getProfilePageData(params: { period: TimePeriod }): Promise<ProfilePageData>;
}
...
export async function linkNewWallet(walletAddress: string) {
  const challengeResponse = await client.api.profile[
    "linked-wallets"
  ].challenge.$post({
    json: {
      walletAddress,
  ...
  if (!challengeResponse.ok) {
    throw new Error(
      `Failed to request wallet link challenge: ${challengeResponse.status}`,
    );
  }
...
class ApiProfileDataProvider implements ProfileDataProvider {
  async getProfilePageData(params: {
    ...
    const linkedWalletRows = linkedWalletAddresses.map(
      (walletAddress, index) => ({
        walletId: walletAddress,
        walletAddress,
        walletLabel:
          adapted.wallets.linkedWalletRows[index]?.walletLabel ?? walletAddress,
        netWorthUsd: adapted.wallets.linkedWalletRows[index]?.netWorthUsd ?? 0,
        lastActiveAt:
          adapted.wallets.linkedWalletRows[index]?.lastActiveAt ??
