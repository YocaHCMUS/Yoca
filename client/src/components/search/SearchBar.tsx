...
export type { TokenResult, PoolResult, SearchResult };
...
export function SearchBar({ onClose }: SearchBarProps) {
    ...
    const [lastFocusedToken, setLastFocusedToken] = useState<TokenResult | null>(null);
    ...
    const doSearch = useCallback(async (q: string) => {
        ...
        setLoading(true);
        try {
            const res = await client.api.search.$get({ query: { q } });
            if (res.ok) {
                const data = await res.json();
                setTokenResults(data.tokens ?? []);
                setPoolResults(data.pools ?? []);
            }
        } catch {
            // silent
        } finally {
    ...
    const handleInput = (value: string) => {
        ...
        debounceRef.current = setTimeout(() => doSearch(value), 320);
    };
    ...
    const selectPool = useCallback(
        (pool: PoolResult) => {
            ...
            const addr = tokenId?.includes("_") ? tokenId.split("_")[1] : tokenId;
            if (addr && pool.attributes) {
                navigate(`/tokens/${addr}/${pool.attributes.address}`);
                onClose();
            }
        },
        [navigate, onClose],
    ...
    const isEmpty = !loading && query.trim() && totalResults === 0;
    const showEmpty = !loading && !query.trim();
    ...
    const activeToken =
        activeIdx >= 0 && activeIdx < tokenResults.length ? tokenResults[activeIdx] : null;
                                        ...
                                        {poolResults.map((pool, idx) => {
                                            ...
                                            return (
                                                <PoolResultItem
                                                    ...
                                                    pool={pool}
                                                    isFocused={
                                                        actualIdx === focusedIdx ||
                                                        actualIdx === hoveredIdx
                                                    }
                                                    onSelect={selectPool}
