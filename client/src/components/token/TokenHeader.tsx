...
import { Copy, LogoDiscord } from "@carbon/icons-react";
...
export const TokenHeader = ({ name, symbol, address, imageUrl, coinGeckoId, discordInvite, websiteUrl, twitterHandle, tokenAge }: TokenHeaderProps) => {
    ...
    const openCoinGecko = () => {
        if (coinGeckoId) {
            ...
            window.open(`https://www.coingecko.com/en/coins/${coinGeckoId}`, '_blank');
        } else {
            // Fallback to contract address (may or may not work)
            window.open(`https://www.coingecko.com/en/coins/solana/contract/${address}`, '_blank');
        }
    };
