...
export function aggregateTradesForScatter(
    ...
    for (const trade of validTrades) {
        ...
        const timeKey =
            Math.floor(trade.unixTimeMs / timeBucketMs) * timeBucketMs;
...
export function getDefaultAggregationForDayRange(days: 7 | 30 | 90): TradeAggregationConfig {
    if (days === 7) {
...
export function TimeSeriesTradesScatterChart({
    ...
    const chartData = useMemo(() => {
        ...
        return sorted.map((point) => [point.unixTimeMs, point.value] as [number, number]);
    }, [data, isDataSorted]);
    ...
    const option = useMemo((): EChartsOption => {
        ...
        const endTime = endTimeCandidates.length ? Math.max(...endTimeCandidates) : startTime;
        const range = Math.max(1, endTime - startTime);

        const trendColor =
            chartData.length >= 2 && chartData[chartData.length - 1][1] >= chartData[0][1]
                ? tokens.success
                : tokens.error;
        ...
        return {
            ...
            tooltip: {
                ...
                formatter: (rawParams) => {
                    ...
                    if (params.seriesType === "line") {
                        ...
                        const valStr = valueFormatter ? valueFormatter(val) : fmt.num.compact.currency(val);
              ...
              <div style="padding:8px 12px; border-radius:4px; background:${tokens.bgInverse}">
                ...
                <div style="font-size:14px; font-weight:700; color:${tokens.textInverse}">${valStr}</div>
                    ...
                    const side = params.data.side === "buy" ? tr("walletPage.buy") : tr("walletPage.sell");
            ...
            yAxis: {
                ...
                min: (value) => {
                    ...
                    const spread = value.max - value.min || Math.max(Math.abs(min) * 0.1, 1);
                ...
                max: (value) => {
                    ...
                    const spread = value.max - value.min || Math.max(Math.abs(max) * 0.1, 1);
                    ...
                    markLine: {
                        ...
                        data: markLines.map((markLine) => ({
                            ...
                            label: {
                                ...
                                position:
                                    markLines.length === 2
                                        ? markLine.value > (markLines[0] === markLine ? markLines[1].value : markLines[0].value)
                                            ? "insideEndTop"
                                            : "insideEndBottom"
                                        : "insideEndBottom",
