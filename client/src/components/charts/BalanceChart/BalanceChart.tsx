...
async function fetchBalanceTrendWithCache(
  query: BalanceRequestParams,
...
function parseTokenSymbolFromSeriesName(seriesName: string): string | null {
  const tokenMatch = seriesName.match(
    /\s([A-Za-z0-9._-]+)\s+\((?:USD|units)\)$/i,
  );
  if (tokenMatch?.[1]) {
    return tokenMatch[1].toUpperCase();
  }

  const singleMatch = seriesName.match(
    /^([A-Za-z0-9._-]+)\s+\((?:USD|units)\)$/i,
  );
  if (singleMatch?.[1]) {
...
function mergeLoadingState(
  ...
  if (
    ...
    (hasTokenSelection && tokenState.status === "loading")
  ) {
    return {
      status: "loading",
  ...
  if (
    generalState.status === "refreshing" ||
    (hasTokenSelection && tokenState.status === "refreshing")
  ) {
    return {
      status: "refreshing",
      retryCount: Math.max(generalState.retryCount, tokenState.retryCount),
...
function BalanceChart({
  ...
  const [prefetchedTokenSeriesBySymbol, setPrefetchedTokenSeriesBySymbol] =
    useState<Record<string, BalanceSeries>>({});
  const [chartWindowDays, setChartWindowDays] = useState<7 | 30>(() =>
    getWindowDaysFromTimePeriod(initialFilters.timePeriod),
  );
  ...
  const tokenQuery = useMemo<BalanceRequestParams>(
    () => ({
      ...
      timezone,
      tokens:
        activeTokenTags.length > 0 ? activeTokenTags.join(",") : undefined,
    }),
  ...
  const generalData = useMemo<BalanceChartDisplayData | null>(() => {
    if (
      generalRawData &&
      !("error" in generalRawData) &&
      isBalanceChartDisplayData(generalRawData)
    ) {
      return {
        ...generalRawData,
  ...
  const tokenData = useMemo<BalanceChartDisplayData | null>(() => {
    if (
      tokenRawData &&
      !("error" in tokenRawData) &&
      isBalanceChartDisplayData(tokenRawData)
    ) {
      return {
        ...tokenRawData,
  ...
  useEffect(() => {
    ...
    const prefetchTokenSeries = async () => {
      ...
      for (const tokenChunk of chunks) {
        ...
        const usdSeries = response.series.filter(
          (series) => series.unit === "USD",
        );
  ...
  const tokenOptions = useMemo(() => {
    ...
    if (isMultiWallet && Array.isArray(generalData?.wallets)) {
      generalData.wallets.forEach((walletAddress, index) => {
        const fallbackLabel =
          generalData.series[index]?.name || toAddressLabel(walletAddress);
        const walletLabel =
  ...
  const displaySeries = useMemo<DisplaySeries[]>(() => {
    ...
    const generalUsdSeries = generalData.series.filter(
      (series) => series.unit !== "TOKEN",
    );
    const tokenUsdSeries = (tokenData?.series ?? []).filter(
      (series) => series.unit === "USD",
    );
    ...
    if (isMultiWallet) {
      ...
      return filteredTokenSeries.map((series, index) => {
        const walletAddress =
          tokenData?.wallets?.[index] ?? generalData.wallets?.[index];
        const label = walletAddress
          ? walletMeta[walletAddress]?.identityName ||
            walletMeta[walletAddress]?.label ||
            toAddressLabel(walletAddress)
    ...
    for (const [symbol, series] of Object.entries(
      prefetchedTokenSeriesBySymbol,
    )) {
  ...
  const windowedDisplaySeries = useMemo<DisplaySeries[]>(() => {
    ...
    const latestTimestamp = displaySeries.reduce((maxTimestamp, series) => {
      ...
      return lastPoint
        ? Math.max(maxTimestamp, lastPoint.timestamp)
        : maxTimestamp;
  ...
  const chartOption = useMemo((): EChartsOption | null => {
    ...
    return {
      ...
      legend: getConditionalLegend(
        chartTheme,
        windowedDisplaySeries.map((series) => series.label),
        2,
        false,
      ),
      xAxis: {
  ...
  const summaryRows = useMemo(() => {
    ...
    return selectedTags.map((tag) => {
      ...
      return {
        ...
        label:
          tag === ALL_TAG
            ? tokenMeta[ALL_TAG]?.symbol || "All"
            : tokenMeta[tag]?.symbol || tag,
  ...
  return (
    ...
    >
      <div
        className={`${sharedStyles.chartControls} ${sharedStyles.balanceChartControlArea}`}
      >
        <div
        ...
        >
          <div className={sharedStyles.balanceChartControlInputGroup}>
            <label htmlFor={dataListId}>
              {isMultiWallet
                ? tr("charts.balanceChart.selectModeTokenLabel")
                : tr("charts.balanceChart.selectTokenLabel")}
            </label>
            <select
              id={dataListId}
              value={selectorValue}
            ...
            >
              {isMultiWallet
                ? tr("charts.balanceChart.switch")
                : tr("charts.balanceChart.add")}
            </button>
          </div>
        ...
        <div className={sharedStyles.balanceChartTagRow}>
          {summaryRows.map((row) => {
            const isRemovableTag =
              !isMultiWallet && selectedTags.includes(row.key);
            const canDismiss = isRemovableTag && selectedTags.length > 1;
            const changeText = row.change
              ? `${row.change.pct >= 0 ? "+" : ""}${row.change.pct.toFixed(2)}%`
              : tr("charts.balanceChart.notAvailable");
            const deltaText = row.change
              ? row.change.deltaLabel
              : tr("charts.balanceChart.noDataDelta");
            ...
            return (
              <div key={row.key} className={sharedStyles.balanceChartTag}>
                <div className={sharedStyles.balanceChartTagMain}>
                  {row.logoUri ? (
                    <img
                      src={row.logoUri}
                      alt={row.label}
                      className={sharedStyles.balanceChartTagIcon}
                    />
                  ) : (
                  ...
                  <span className={sharedStyles.balanceChartTagLabel}>
                    {row.label}
                  </span>
                </div>
                ...
                <div className={sharedStyles.balanceChartTagMetrics}>
                  ...
                  <span className={sharedStyles.balanceChartTagDelta}>
                    {deltaText}
                  </span>
                </div>
                ...
                {isRemovableTag && (
                  <button
                    ...
                    title={
                      canDismiss
                        ? tr("charts.balanceChart.removeTag")
                        : tr("charts.balanceChart.atLeastOneTagRequired")
                    }
                    data-html2canvas-ignore="true"
                  >
                    <Close size={12} />
