...
function normalizeSeriesData(points: Array<{ timestamp: number; value: number }>): BalanceSeriesPoint[] {
...
export function WalletSingleBalanceChart({
    ...
    const [chartWindowDays, setChartWindowDays] = useState<7 | 30>(() => initialFilters.timePeriod?.toUpperCase() === '7D' ? 7 : 30);
    const [activeWalletAddress, setActiveWalletAddress] = useState('');
    const [walletStatsByAddress, setWalletStatsByAddress] = useState<Record<string, WalletStats>>({});
    ...
    const walletAddresses = useMemo(
        () => Array.from(new Set((filters.wallets ?? []).map((address) => address.trim()).filter((address) => address.length > 0))),
        [filters.wallets],
    );
    ...
    useEffect(() => {
        ...
        setActiveWalletAddress((prev) => (prev && walletAddresses.includes(prev) ? prev : walletAddresses[0]));
    }, [walletAddresses]);
    ...
    const { data: rawData, loadingState, refetch } = useStandardChartController<BalanceTrendData, BalanceRequestParams>({
    ...
    const data = useMemo<BalanceChartDisplayData | null>(() => {
        if (rawData && !('error' in rawData) && isBalanceChartDisplayData(rawData)) {
            return {
                ...rawData,
    ...
    const selectedWalletPoints = useMemo<BalanceSeriesPoint[]>(() => {
        ...
        const selectedSeries = data.series.find((series) => series.unit !== 'TOKEN');
    ...
    const windowedPoints = useMemo(() => {
        ...
        const latest = selectedWalletPoints[selectedWalletPoints.length - 1].timestamp;
        const cutoff = latest - chartWindowDays * ONE_DAY_MS;
        const filtered = selectedWalletPoints.filter((point) => point.timestamp >= cutoff);
    ...
    const option = useMemo<EChartsOption | null>(() => {
        ...
        return {
            ...
            tooltip: {
                ...base.tooltip,
                trigger: 'axis',
                formatter: (params) => formatAxisTooltip(
                    params,
    ...
    const tableHeaders = useMemo<TableColumnHeader[]>(() => [
        { header: tr('charts.walletSingleBalanceChart.walletTable.wallet'), minWidth: '10rem' },
    ], [tr]);

    const tableRows = useMemo<any[][]>(() => walletRows.map((wallet) => {
    ...
    const headerControls = (
        <PeriodSelector
            value={(filters.timePeriod?.toUpperCase() === '7D' ? '7D' : '30D') as TimePeriod}
            onChange={(period) => {
    ...
    const isEmpty = walletRows.length === 0 || !selectedWallet || windowedPoints.length === 0;
            ...
            <div className={styles.layout}>
                <div className={styles.tablePanel}>
                    <Table
                        ...
                        onRowClick={(row) => setActiveWalletAddress(String((row[0] as WalletIdentityCellValue | undefined)?.walletAddress ?? ''))}
                        rowClassName={(row) => (
                            String((row[0] as WalletIdentityCellValue | undefined)?.walletAddress ?? '') === activeWalletAddress
                    ...
                    {isEmpty
                        ? (
                            <div className={styles.emptyState}>
                                <p className={styles.emptyStateTitle}>{tr('charts.noDataTitle')}</p>
                                <p className={styles.emptyStateMessage}>{tr('charts.noDataMessage')}</p>
                            </div>
                        )
                        : (
                            <ReactECharts
