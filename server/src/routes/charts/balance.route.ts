...
const app = new Hono().get(
  "/",
  validate("query", balanceRequestSchema),
  async (c) => {
    try {
      ...
      if (!params.wallets) {
        return c.json(
          { error: "Validation error", message: "wallets is required" },
          400,
        );
      }
        ...
        return c.json(
          {
            ...
            message: "Invalid wallet addresses provided",
          },
          400,
        );
      }
      ...
      if (tokenSelectors.length === 0) {
        const allHistories = await mapWithConcurrency(
          ...
          async (address) =>
            getWalletBalanceHistory(address, params.timePeriod),
        );
        ...
        const series =
          walletAddresses.length === 1
            ...
            : walletAddresses.map((address, index) => ({
                ...
                data: allHistories[index].map((point) => ({
                  timestamp: point.timestamp,
                  value: point.value,
                })),
                seriesType: "line" as const,
      ...
      const tokenMeta = Object.fromEntries(
        selectedTokenSymbols.map((symbol) => {
          ...
          const pairMeta = pairResults.find(
            (result) => result.tokenSymbol.toUpperCase() === symbol,
          );
          ...
          return [
            symbol,
            {
              ...
              tokenAddress:
                selectorMeta?.tokenAddress ?? pairMeta?.tokenAddress,
            },
          ];
        }),
      );
      ...
      const series = pairs.flatMap(({ address }, index) => {
        ...
        const prefix =
          walletAddresses.length > 1 ? `${address.substring(0, 8)}... ` : "";
        return [
          {
      ...
      console.error(
        "[BalanceChart] Error fetching wallet balance history:",
        error,
      );
      return c.json(
        {
