...
import {
  ...
  normalizeMint,
  normalizePriceTimeline,
  toCurrentPriceFallback,
} from "./walletData.core.js";
import {
  normalizeBalanceDelta,
  roundUsd,
} from "./walletNormalization.utils.js";
    ...
    Promise.all(
      tokenAddressList.map(async (tokenAddress) => {
        const chartRows = useHourlyChart
          ? await getHourlyTokenMarketChart(
              tokenAddress,
              daysSpan,
              cgIdByAddress,
            )
          : await getDailyTokenMarketChart(
              tokenAddress,
              daysSpan,
              cgIdByAddress,
            );
