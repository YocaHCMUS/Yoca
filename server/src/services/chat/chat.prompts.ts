...
export function buildToolSelectionPrompt(
  ...
  lines.push(
    ...
    "If one tool needs a value from another tool, use resolveFrom: {\"resolveFrom\": {\"tool\": \"search_token\", \"input\": {\"query\": \"example\"}, \"pick\": \"tokens[0].address\"}}. Replace input and pick based on the resolver tool's schema. The resolver tool MUST be in the same tools array.",
