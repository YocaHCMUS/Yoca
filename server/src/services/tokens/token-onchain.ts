...
async function fetchOnchainTokenData(
  ...
  const poolTxs24h = topPool?.transactions?.h24 || {
    buys: 0,
    sells: 0,
    buyers: 0,
    sellers: 0,
  ...
  return {
    ...
    totalSupply: parseFloat(
      token.normalized_total_supply || token.total_supply || "0",
    ),
    volume5m: parseFloat(volumeUsd.m5 || "0"),
    volume1h: parseFloat(volumeUsd.h1 || "0"),
    volume24h: parseFloat(volumeUsd.h24 || "0"),
    priceChange5m: parseFloat(
      topPool?.price_change_percentage?.m5 || priceChange.m5 || "0",
    ),
    priceChange1h: parseFloat(
      topPool?.price_change_percentage?.h1 || priceChange.h1 || "0",
    ),
    priceChange24h: parseFloat(
      topPool?.price_change_percentage?.h24 || priceChange.h24 || "0",
    ),
