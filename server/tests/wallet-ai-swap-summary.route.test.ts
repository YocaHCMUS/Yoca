...
vi.mock("@sv/middlewares/validation.js", async () => {
  ...
  return {
    ...
    addressSchema: z.object({ address: z.string() }),
    solanaBase58Schema: z.string(),
    solanaSignatureSchema: z.string(),
    walletTokenTradesSchema: z.object({
      walletAddress: z.string(),
      tokenAddress: z.string(),
    }),
    validate:
      (target: "json" | "query" | "param", schema: z.ZodType) =>
      async (c: TestContext, next: Next) => {
        const value =
          target == "json"
            ? await c.req.json()
            : target == "query"
        ...
        if (!parsed.success) {
          return c.json({ errorCode: "VALIDATION_ERR" }, 422);
        }
...
import app from "@sv/routes/wallets.js";
...
type RoutePath = "/ai-swap-summary" | "/ai-swap-summary/token";
