...
- [x] **BẮT BUỘC — Deployment:** đã có ảnh Render dashboard thật cho cả 2 service, cả hai ở trạng thái Live: `render-deployment-frontend.jpg` (Static Site `yoca-frontend`, phục vụ tại `yoca.id.vn`) và `render-deployment-backend.jpg` (Web Service `yoca-backend` tại `yoca-backend.onrender.com`, log cho thấy request `GET /api` thật trả về 200). Đã chèn vào chapter4.tex, đồng thời bỏ các câu hedge cũ ("chưa có bằng chứng vận hành", "Render vẫn cần được xác minh") ở chapter4.tex và Chapter5/chapter5.tex (Kết luận) vì nay đã có bằng chứng trực tiếp.
...
- [x] `images/chapter5/github-actions-ci.png`
- [x] `images/chapter5/render-deployment-frontend.jpg` + `render-deployment-backend.jpg`
