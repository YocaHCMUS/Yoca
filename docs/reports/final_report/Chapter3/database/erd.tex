...
Sơ đồ tổng quan ở Hình~\ref{fig:db_overview} giúp người đọc thấy toàn bộ kiến trúc, còn các hình dưới đây là những lát cắt có thể đọc được trên trang dọc A4. Cách tách này cũng cho phép đổi một miền cache mà không phải vẽ lại một ERD ngang quá lớn.
...
Hình~\ref{fig:db_alert_history} tách định nghĩa cảnh báo khỏi trạng thái, delivery target và điều kiện kích hoạt. \texttt{alert\_history} lưu sự kiện sau khi ít nhất một kênh gửi thành công, đồng thời hỗ trợ chống ghi trùng, phân trang và trạng thái đã đọc. API chỉ cho phép người dùng truy cập hoặc cập nhật lịch sử thuộc tài khoản của mình; giao diện Profile sử dụng cùng API này để hiển thị lịch sử thực tế thay cho dữ liệu mock ban đầu.
...
Hình~\ref{fig:db_wallet_analytics} dùng \texttt{WALLET\_ADDRESS} như một thực thể logic, không phải bảng vật lý. \texttt{wallet\_overview\_cache} cung cấp bản tổng hợp cho trang overview; \texttt{wallet\_analyses} lưu kết quả Mobula theo ví và kỳ phân tích; \texttt{wallet\_token\_details} giữ PnL ở cấp token. Identity, nguồn cấp vốn đầu tiên và kết quả AI được tách thành cache riêng vì có nguồn, chi phí và TTL khác nhau.
