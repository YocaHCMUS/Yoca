...
Cấu hình runtime được truyền qua biến môi trường. Client chỉ nhận các giá trị có tiền tố \texttt{VITE\_} và được phép xuất hiện trong browser; khóa provider, chuỗi kết nối PostgreSQL, JWT secret và Stripe secret chỉ thuộc server. Repository cung cấp script riêng cho dev, test, lint, build và migration. Trong phạm vi báo cáo, nhóm không đưa nội dung tệp \texttt{.env} hoặc giá trị secret vào phụ lục hay ảnh minh họa.

\subsection{Tích hợp mã nguồn và trạng thái CI/CD}

Ở nhánh đang dùng để hoàn thiện báo cáo, quy trình có thể xác nhận là tích hợp thủ công: cập nhật dependency, chạy test phù hợp với phần thay đổi, rà migration khi schema thay đổi và chạy thử client/server. Script gốc có thể khởi động hai workspace bằng \texttt{npm run dev}; server cung cấp script Drizzle cho generate/migrate/push, còn mỗi workspace có lệnh test riêng.

Nhóm đang xây dựng pipeline CI/CD ở một nhánh khác. Vì workflow đó chưa có trong source đang được rà và chưa có kết quả chạy cuối cùng, báo cáo chưa mô tả trigger, job hay deployment tự động như một kết quả đã hoàn thành. Sau khi pipeline được merge và chạy ổn định, mục này cần được cập nhật lại dựa trên workflow thật, bao gồm kiểm tra mã nguồn, test, migration, quản lý environment/secret và điều kiện triển khai.

\subsection{Triển khai và kiểm tra sau triển khai}

Quy trình hiện có ở mức thủ công gồm chuẩn bị biến môi trường, cài dependency, cập nhật lược đồ PostgreSQL theo migration, khởi động server rồi client và kiểm tra endpoint trạng thái \texttt{/api}. Sau đó nhóm cần smoke test hành trình Market Overview -- Token Pool -- Token Overview -- Wallet -- User -- AI, đồng thời kiểm tra một trường hợp provider lỗi để bảo đảm giao diện không giữ trạng thái tải vô hạn.

Đây là quy trình mục tiêu để kiểm tra một bản triển khai, chưa phải bằng chứng về một môi trường staging, rollback hay monitoring tự động. Các bước cụ thể sẽ được đồng bộ lại khi nhánh CI/CD hoàn tất; báo cáo chỉ ghi kết quả deployment khi có workflow và lần chạy có thể đối chiếu.
...
% TODO(OPTIONAL-SCREENSHOT): đặt file images/chapter5/auth-modal.png; ảnh phải thấy email, Google và ví Solana, không dùng email/ví thật.
...
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/market-overview.png; phải thấy market tab, bộ lọc, bảng dữ liệu và một token/pool có thể điều hướng.
...
% TODO(SCREENSHOT-DECISION): không chụp riêng Search nếu market-overview.png hoặc token-overview.png đã thấy kết quả token/pool/wallet.
...
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/token-overview.png; phải thấy token identity/stats, chart và một tab nội dung có dữ liệu.
...
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/token-pool.png; phải thấy pool selector, market metrics, chart và recent transactions.
...
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/wallet-analysis.png; dùng ví demo và phải thấy holdings, PnL/volume theo kỳ cùng balance chart.
...
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/profile-watchlist.png; chỉ dùng dữ liệu demo, ưu tiên thấy watchlist/linked wallet và trạng thái subscription, không lộ email thật.
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/ai-limit.png; phải thấy một phản hồi AI có dữ liệu nguồn hoặc trạng thái giới hạn lượt dùng rõ ràng.
...
% TODO(OPTIONAL-SCREENSHOT): đặt file images/chapter5/wash-trading.png nếu demo ổn định; phải thấy graph, risk metrics và phần giải thích, không dùng ảnh mock.

\subsection{Quá trình đồng nhất hóa giao diện}
...
% TODO(REQUIRED-SCREENSHOT): đặt hai file images/chapter5/localization-en-usd.png và localization-vi-vnd.png; chụp cùng viewport/cùng dữ liệu, phải thấy chuỗi, ngày giờ và ít nhất một giá trị USD/VND tương ứng.
...
\begin{equation}
    PnL_d = (B_{d,end} - B_{d,start}) - NetFlow_d,
\end{equation}
trong đó $B$ là tổng giá trị ví và $NetFlow_d$ là giá trị tài sản nhận vào trừ tài sản chuyển ra trong ngày. Kết quả này phản ánh biến động danh mục sau khi loại ảnh hưởng của nạp/rút, không đồng nhất hoàn toàn với realized PnL theo từng giao dịch.
...
\subsection{Giá trị kỹ thuật sau khi thu hẹp phạm vi tự tính toán}
...
Để giải quyết vấn đề này, trước khi gửi yêu cầu đến mô hình AI, Yoca xây dựng một bối cảnh có cấu trúc gồm các chỉ số tổng hợp, giao dịch tiêu biểu, khoảng thời gian phân tích và các bằng chứng liên quan. Ngữ cảnh này được định nghĩa bằng kiểu dữ liệu riêng cho từng miền, ví dụ \texttt{TokenAiContext} cho token và các DTO tương ứng cho ví, thay vì truyền thẳng response thô của provider vào prompt. Việc này giúp mô hình tập trung vào những thông tin quan trọng thay vì xử lý toàn bộ dữ liệu blockchain.

Vì lịch sử hội thoại và dữ liệu do các tool trả về đều là nội dung không hoàn toàn đáng tin cậy, hệ thống bọc các phần này trong dấu phân định rõ ràng kèm chỉ dẫn rằng mô hình chỉ tuân theo hướng dẫn hệ thống, không thực thi chỉ dẫn nằm trong phần dữ liệu đó. Đây là một ranh giới prompt-injection ở mức thực dụng, không phải cơ chế đã được kiểm chứng độc lập. Sau khi AI tạo kết quả, phần diễn giải luôn được hiển thị cùng với bảng dữ liệu hoặc biểu đồ tương ứng để người dùng dễ dàng đối chiếu. Đối với mô-đun wash trading, kết quả được liên kết trực tiếp với các đặc trưng và các ví đáng ngờ; trong khi ở các trang phân tích ví và token, AI chỉ diễn giải dữ liệu của đối tượng đang được quan sát. 
...
Yoca sử dụng Vitest ở cả client và server. Client chạy trong môi trường jsdom và kết hợp Testing Library để kiểm tra component, thao tác người dùng và trạng thái hiển thị. Server chạy trong môi trường Node; phần lớn test cô lập route hoặc service bằng mock cho database, provider, Stripe, Helius và AI. Cách tổ chức này có giá trị đối với regression ở logic nội bộ, nhưng không tương đương kiểm thử end-to-end với PostgreSQL và external API thật.
...
Ngày 11/07/2026, nhóm chạy hai lệnh \texttt{npm test -w client -- --run} và \texttt{npm test -w server} trên source hiện tại. Kết quả được trình bày trong Bảng~\ref{tab:test_results}; đây là số liệu của đúng lần chạy này, không phải coverage hay tỷ lệ ổn định dài hạn.

\begin{table}[H]
\centering
\small
\begin{tabularx}{\textwidth}{|L{2.5cm}|C{2.2cm}|C{2.2cm}|L{6.7cm}|}
\hline
\textbf{Suite} & \textbf{Test file} & \textbf{Test case} & \textbf{Kết quả} \\
\hline
Client & 11/15 đạt & 155/172 đạt & 17 test thất bại ở table localization expectation, chart click handler, Quick AI mock và ThemeProvider của Wallet Holdings. \\
\hline
Server & 24/27 đạt & 181/189 đạt & 8 test thất bại ở chat tool metadata/prompt và đường lỗi Zerion balance: service trả \texttt{null}/500 thay cho typed upstream error/502 như test kỳ vọng. \\
\hline
\end{tabularx}
...
Kết quả cho thấy repository đã có nền tảng kiểm thử đáng kể, đồng thời cũng xác nhận test suite chưa ở trạng thái xanh hoàn toàn. Một phần lỗi client xuất hiện sau thay đổi localization, theme hoặc API mock nhưng test chưa được cập nhật. Nhóm lỗi server đáng chú ý hơn vì liên quan đến contract lỗi: malformed/empty/error response từ Zerion đang bị hạ thành \texttt{null} hoặc HTTP 500 ở một số đường, trong khi test yêu cầu typed upstream failure và HTTP 502. Báo cáo không xem các test thất bại này là kết quả đạt yêu cầu; chúng là backlog cần xử lý trước khi dùng pipeline CI làm cổng merge.
...
Kết quả chạy test cho thấy nhiều luồng auth, payment, alert, AI, component và Wallet đã có regression test, nhưng cả client lẫn server vẫn còn test thất bại cần xử lý. Cùng với việc CI/CD đang được phát triển ở nhánh khác và chưa có benchmark/E2E đầy đủ, đây là giới hạn phải được giữ rõ khi đánh giá mức độ hoàn thiện. Những kết quả và tồn tại này là cơ sở để Chương 6 tổng kết đóng góp, giới hạn và hướng phát triển tiếp theo cho Yoca.
