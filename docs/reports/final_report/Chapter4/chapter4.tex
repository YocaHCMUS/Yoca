...
Workflow CI được đặt tại \texttt{.github/workflows/ci.yml} và chạy khi có thay đổi được đẩy lên nhánh \texttt{main}, \texttt{dev} hoặc khi pull request hướng vào \texttt{main}. Pipeline tách thành sáu job độc lập: typecheck, lint, test server, test client, build server và build client. Mỗi job checkout cùng revision, cài dependency theo npm workspace rồi thực hiện script tương ứng. Cách tách này giúp nhóm nhìn rõ lỗi thuộc hợp đồng kiểu, quy tắc mã nguồn, regression test hay production build, thay vì gộp mọi bước vào một lệnh khó chẩn đoán.
...
Sau triển khai, nhóm kiểm tra endpoint \texttt{/api}, CORS giữa hai domain, SPA rewrite của Static Site và hành trình Market Overview -- Token Pool -- Token Overview -- Wallet -- User -- AI. Các callback Google/Stripe, Helius webhook, Alert History và kết nối Supabase cần smoke test bằng cấu hình thật. Free tier có thể tạo cold start hoặc giới hạn tài nguyên, nên báo cáo không xem thời gian phản hồi của lần gọi đầu tiên là một SLA ổn định và không tuyên bố có rollback hay monitoring tự động khi chưa có bằng chứng vận hành.

% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/github-actions-ci.png; phải thấy các job CI đạt và không lộ secret.
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/render-deployment.png; phải thấy Static Site và Web Service deploy thành công, không lộ deploy hook/environment.
...
\begin{figure}[H]
    ...
    \caption{Market Overview: nhóm Trending, bộ lọc theo khung thời gian và bảng token/pool có thể điều hướng tiếp}
...
\begin{figure}[H]
    ...
    \caption{Token Overview: identity, thống kê giá/thị trường và biểu đồ giá của một token}
...
\begin{figure}[H]
    ...
    \caption{Tab Holders: danh sách địa chỉ nắm giữ lớn và biểu đồ phân bố nguồn cung theo bốn lớp}
    \label{fig:token-overview-holders}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/chapter5/ai-limit.png}
    \caption{Trợ lý AI trả lời câu hỏi về token kèm trạng thái giới hạn lượt hỏi còn lại trong ngày (gói Free)}
    \label{fig:ai-limit}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Token Pool: pool selector, market metrics và biểu đồ nến của cặp giao dịch đang chọn}
...
\begin{figure}[H]
    ...
    \caption{Wallet Overview: win rate, tổng tài sản, PnL, khối lượng giao dịch, biểu đồ số dư và phân bổ tài sản của một ví demo}
...
\begin{figure}[H]
    ...
    \caption{Phân tích chi tiết theo từng vị thế token: số dư, lãi/lỗ đã và chưa thực hiện, khối lượng và giá mua/bán trung bình}
...
% TODO(REQUIRED-SCREENSHOT): đặt file images/chapter5/alert-history.png; phải thấy notification thật và trạng thái read/unread, không lộ email/webhook/signature/ví nhạy cảm.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.66\textwidth]{images/chapter5/localization-en-usd.png}\\[3pt]
    {\small (a) Giao diện tiếng Anh, đơn vị USD}\\[8pt]
    \includegraphics[width=0.66\textwidth]{images/chapter5/localization-vi-vnd.png}\\[3pt]
    {\small (b) Giao diện tiếng Việt, đơn vị VND}
    \caption{Cùng trang Wallet Overview ở hai locale: chuỗi giao diện, định dạng ngày/thời lượng và giá trị tài chính được quy đổi và làm tròn theo VND}
...
\subsection{Giảm thiểu prompt injection và kết quả AI sai cấu trúc}

Các luồng AI có một ranh giới tin cậy khác với API thông thường: câu hỏi hiện tại, lịch sử hội thoại và nội dung do search/provider trả về đều có thể chứa văn bản giống câu lệnh. Trong Wallet Chat, nhóm chúng em đặt quy tắc bất biến trong \texttt{systemInstruction}, còn user message, history và tool result được đóng gói trong các khối \texttt{UNTRUSTED} có marker kết thúc rõ ràng. Bước chọn tool chỉ chấp nhận tên nằm trong allowlist; dữ liệu đầu vào tiếp tục được chuẩn hóa và giới hạn trước khi gọi service.
